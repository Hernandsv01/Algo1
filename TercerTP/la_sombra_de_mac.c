...
const char* FORMATO_ARCHIVO_NIVEL1 = "N1";
const char* FORMATO_ARCHIVO_NIVEL2 = "N2";
const char* FORMATO_ARCHIVO_NIVEL3 = "N3";
const char* FORMATO_ARCHIVO_POZOS = "POZOS";
const char* FORMATO_ARCHIVO_VELAS = "VELAS";
const char* FORMATO_ARCHIVO_INTERRUPTORES = "INTERRUPTORES";
const char* FORMATO_ARCHIVO_PORTALES = "PORTALES";
const char* FORMATO_ARCHIVO_MONEDAS = "MONEDAS";
const char* FORMATO_ARCHIVO_ESCALERAS = "ESCALERAS";
const char* FORMATO_ARCHIVO_LLAVES = "LLAVES";
const char* NOMBRE_ARCHIVO_CONFIG = "config.txt";
const char* MODO_READ = "r";
...
const int DIST_MINIMA_VICTORIA = 1;

const int ESTADO_NIVEL_JUGANDO = 0;
const int ESTADO_NIVEL_GANADO = 1;
const int ESTADO_JUEGO_JUGANDO = 0;
 ...
 * Post: 
 */
void llenar_matriz_elementos_constantes(int matriz[MAX_ELEMENTOS][MAX_ELEMENTOS]){
	//Pensar otra forma de hacer esto, se ve horrible
	matriz[0][0] = MAX_POZOS_NVL_1;
	matriz[1][0] = MAX_POZOS_NVL_2;
	matriz[2][0] = MAX_POZOS_NVL_3;

	matriz[0][1] = MAX_VELAS_NVL_1;
	matriz[1][1] = MAX_VELAS_NVL_2;
	matriz[2][1] = MAX_VELAS_NVL_3;

	matriz[0][2] = MAX_INTERRUPTORES_NVL_1;
	matriz[1][2] = MAX_INTERRUPTORES_NVL_2;
	matriz[2][2] = MAX_INTERRUPTORES_NVL_3;

	matriz[0][3] = MAX_PORTALES_NVL_1;
	matriz[1][3] = MAX_PORTALES_NVL_2;
	matriz[2][3] = MAX_PORTALES_NVL_3;

	matriz[0][4] = MAX_MONEDAS_NVL_1;
	matriz[1][4] = MAX_MONEDAS_NVL_2;
	matriz[2][4] = MAX_MONEDAS_NVL_3;

	matriz[0][5] = MAX_ESCALERAS_NVL_1;
	matriz[1][5] = MAX_ESCALERAS_NVL_2;
	matriz[2][5] = MAX_ESCALERAS_NVL_3;

	matriz[0][6] = MAX_LLAVES_NVL_1;
	matriz[1][6] = MAX_LLAVES_NVL_2;
	matriz[2][6] = MAX_LLAVES_NVL_3;
}
 ...
 * Post: 
 */
void llenar_matriz_elementos_archivo(int matriz[MAX_ELEMENTOS][MAX_ELEMENTOS], int nivel, char* elemento, int numero_elementos){

	if(strcmp(elemento, FORMATO_ARCHIVO_POZOS) == 0){
		matriz[nivel][0] = numero_elementos;
	}else if(strcmp(elemento, FORMATO_ARCHIVO_VELAS) == 0){
		matriz[nivel][1] = numero_elementos;
	}else if(strcmp(elemento, FORMATO_ARCHIVO_INTERRUPTORES) == 0){
		matriz[nivel][2] = numero_elementos;
	}else if(strcmp(elemento, FORMATO_ARCHIVO_PORTALES) == 0){
		matriz[nivel][3] = numero_elementos;
	}else if(strcmp(elemento, FORMATO_ARCHIVO_MONEDAS) == 0){
		matriz[nivel][4] = numero_elementos;
	}else if(strcmp(elemento, FORMATO_ARCHIVO_ESCALERAS) == 0){
		matriz[nivel][5] = numero_elementos;
	}else if(strcmp(elemento, FORMATO_ARCHIVO_LLAVES) == 0){
		matriz[nivel][6] = numero_elementos;
	}
}
 ...
 */
void generar_nivel(nivel_t* nivel, int numero_de_nivel, int num_elementos[MAX_ELEMENTOS]){

	//Cargar numero de nivel
	nivel->numero_nivel = numero_de_nivel+1;
	...
	generar_obstaculos(nivel, num_elementos[1], num_elementos[0], num_elementos[2], num_elementos[3]);
	generar_escaleras(nivel->herramientas, &(nivel->tope_herramientas), nivel->paredes, nivel->tope_paredes, num_elementos[5]);
	generar_herramientas(nivel, num_elementos[4], num_elementos[6]);
}
...
void inicializar_juego(juego_t* juego){
	...
	//Verificar archivo config.txt
	int matriz_elementos[MAX_NIVELES][MAX_ELEMENTOS];
	llenar_matriz_elementos_constantes(matriz_elementos);

	FILE* config = fopen(NOMBRE_ARCHIVO_CONFIG, MODO_READ);
	if(config){
		char linea[MAX_ELEMENTOS];
		char* nivel;
		int num_nivel;
		char* elemento;
		int numero_elementos;
		while(!feof(config)){
			fscanf(config, "%s", linea);
			nivel = strtok(linea, "_");
			elemento = strtok(NULL, "=");
			numero_elementos = atoi(strtok(NULL, "\n"));

			if(strcmp(nivel, FORMATO_ARCHIVO_NIVEL1) == 0){
				num_nivel = 0;
			}else if(strcmp(nivel, FORMATO_ARCHIVO_NIVEL2) == 0){
				num_nivel = 1;
			}else if(strcmp(nivel, FORMATO_ARCHIVO_NIVEL3) == 0){
				num_nivel = 2;
			}

			llenar_matriz_elementos_archivo(matriz_elementos, num_nivel, elemento, numero_elementos);
		}
	}
	

	for(int i = 0; i < MAX_NIVELES; i++){
		generar_nivel(&(juego->niveles[i]), i, matriz_elementos[i]);
	}
...
int estado_juego(juego_t juego){
	if(estado_nivel(juego) == 1 && juego.nivel_actual == MAX_NIVELES){
		return ESTADO_JUEGO_GANADO;
	}
	if(juego.personaje.vida == 0){
		return ESTADO_JUEGO_PERDIDO;
	}
	return ESTADO_JUEGO_JUGANDO;
}
...
int estado_nivel(juego_t juego){
	...
	if(juego.nivel_actual == 1){
		if(distancia_manhattan(juego.personaje.posicion, puerta.coordenada) <= DIST_MINIMA_VICTORIA && distancia_manhattan(juego.sombra.posicion, puerta.coordenada) <= 1 && juego.sombra.esta_viva){
			estado = ESTADO_NIVEL_GANADO;
		}else{
			estado = ESTADO_NIVEL_JUGANDO;
		}
	}else{
		if(distancia_manhattan(juego.personaje.posicion, puerta.coordenada) <= DIST_MINIMA_VICTORIA && distancia_manhattan(juego.sombra.posicion, puerta.coordenada) <= 1 && juego.personaje.tiene_llave && juego.sombra.esta_viva){
			estado = ESTADO_NIVEL_GANADO;
		}else{
			estado = ESTADO_NIVEL_JUGANDO;
		}
...
void realizar_jugada(juego_t* juego){
	...
	if(movimiento == VIDA){
		if(juego->personaje.puntos >= COSTO_VIDA && juego->personaje.vida < MAX_VIDAS){
			juego->personaje.puntos -= COSTO_VIDA;
			juego->personaje.vida++;
		}
	}else{
		...
		if(buscar_elemento(juego->niveles[juego->nivel_actual-1].herramientas, juego->niveles[juego->nivel_actual-1].tope_herramientas, juego->personaje.posicion) != -1){
			...
			if(juego->niveles[juego->nivel_actual-1].herramientas[pos_herramienta].tipo == ESCALERA){
				accionar_escalera(&(juego->personaje), &(juego->niveles[juego->nivel_actual-1]), movimiento);
			}
		}
